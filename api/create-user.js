...
export default async function handler(req, res) {
  ...
  const { username, email: providedEmail, password, fullName, role, assignedSiteIds } = req.body || {};
  ...
  const { error: updateErr } = await admin
    .from("profiles").update({ role, full_name: fullName, assigned_site_ids: Array.isArray(assignedSiteIds) ? assignedSiteIds : [] }).eq("id", created.user.id);
  if (updateErr) {
